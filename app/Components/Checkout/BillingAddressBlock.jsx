...
let timer;
export function BillingAddressBlock({className = ''}) {
  ...
  useEffect(() => {
    if (customer?.defaultAddress) {
      setBillingAddress(customer?.defaultAddress);
    }
    return () => {
  ...
  useEffect(() => {
    ...
    if (currentSelected) {
      setBillingAddress(currentSelected);
    }
    ...
    try {
      ...
      if (address1 && zip && city && phone && provinceCode && countryCodeV2) {
        const validationResponse = await verifyShippingAddress({
          ...
          countryCode: billingAddress.countryCodeV2,
        });
        if (validationResponse.validation?.concerns?.length === 0) {
          setVerifiedBillingAddress(billingAddress);
        } else {
  ...
  return (
    <fetcher.Form
      className={`w-full p-4 ${className} bg-[#FAFAFA]`}
      method={
        billingAddress?.id && billingAddress?.id !== 'new' ? 'PUT' : 'POST'
      }
    ...
    >
      ...
      <div className="w-full relative flex flex-col justify-start items-start gap-[11px]">
        ...
        <div className="w-full">
          <div className="max-h-[42px] mdl:max-h-[52px] relative country">
            ...
            <CountryDropdown
              ...
              value={billingAddress?.countryCodeV2}
              id="country"
              name="country"
              priorityOptions={['CA', 'US']}
              defaultOptionLabel={billingAddress?.country}
        ...
        <div className="w-full grid md:grid-cols-2 gap-2">
          <div>
            <div
              className={`bg-[#FFF] min-h-[52px]  rounded flex justify-between border ${errors?.firstName ? 'border-2 border-[#E62828]' : 'border-[#DEDEDE]'}`}
            >
          ...
          <div className="w-full">
            ...
            >
              <div className="w-full flex flex-col justify-center relative">
                ...
                <RegionDropdown
                  country={billingAddress?.countryCodeV2}
                  value={billingAddress?.provinceCode}
                  aria-label="State"
                  ...
                  valueType="short"
                  labelType="full"
                  defaultOptionLabel={billingAddress?.province}
                  placeholder="State"
                  onChange={(val) => handleInputChange(val, 'provinceCode')}
                  className={`focus:outline-none focus:ring-0  w-full border-none ${billingAddress?.province && 'pt-[19.2px]'} cursor-pointer`}
                  onFocus={() => clearErrors()}
                />
              </div>
            </div>
